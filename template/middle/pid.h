...
/* ========================================================================== */
/* pid2_t — 平衡车专用简化 PID（对齐 STM32 demo 公式）                         */
/*                                                                            */
/* 与 pid_t 的关键差异：                                                        */
/*   ─ 不传 dt，假设固定调用周期，增益已吸收 dt 因子（demo 行为）。              */
/*   ─ Ki=0 时 i_term 自动清零（调试关键：避免"积分债务"）。                    */
/*   ─ out_offset 死区补偿：非零输出时叠加固定偏移，突破 TB6612 静摩擦死区，     */
/*     消除"突然无力→又突然冲出"症状（demo 关键创新）。                         */
/*   ─ 微分先行（d on measurement）：Kd 作用于 actual 变化率而非 error，        */
/*     避免 setpoint 阶跃引起 D 项冲击。                                        */
/*   ─ i_min/i_max 独立于输出限幅（防 wind-up 的同时允许积分超过输出范围）。    */
/* ========================================================================== */
