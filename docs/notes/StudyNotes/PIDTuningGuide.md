# 平衡车 PID 调参指南（两级级联 + 航向角环）

本文面向当前 `pid2` 分支的控制架构：与 STM32 demo 对齐的 **速度 → 俯仰角度 → PWM** 两级串级，外加 **航向角环** 差分叠加。核心代码在 `template/app/app_balance.c`（`pid2_t`），底层电机在 `template/hardware/bsp_motor.c`，安全保护在 `template/app/app_safety.c`。

> 历史说明：Stage 3.4 曾采用四级级联（含独立角速度内环 `rp`），已在 `pid2` 分支重构为本文架构。演进记录见 [Stage3-BalanceControl.md](../TaskLog/Stage3-BalanceControl.md)。

## 1. 控制结构概览

装车模式由 `main.c` 启动。串口发送 `t` / `test` 进入电机演示，`l` / `load` 返回。
...
| 速度环 | 20 Hz | 目标速度 − 实测速度（归一化 cps） | 目标倾角 (°) | `sp` |
| 航向角环 | 20 Hz | 偏航误差（见 `YAW_SOURCE`） | 差分 PWM permille | `yp` |

**PID 库**：三路控制器均使用 `pid2_update()`（对齐 STM32 `PID_Update`）：积分按拍累加（Ki=0 时自动清零）、**微分先行**（D 作用于测量值变化）、**OutOffset** 死区补偿。
...
- `APP_BALANCE_PITCH_INVERT=1`：MS901M 前后装反时软件翻转。
- 手扶车体前倾时，心跳 `pitch=` 应为正值。
- 方向反了先改宏或确认安装，不要动 PID。
...
- 上电 `APP_SAFETY_STARTUP_FALL_MUTE_MS` 默认 2500 ms 静默，期间不输出 PID。
- 初期可将 `APP_BALANCE_MAX_PWM_PERMILLE` 临时降到 300~500。

### 上电校零

- `app_balance_run()` 入口阻塞约 1.5 s 采 `pitch_offset`（车需静止直立）。
- 失败时日志 `[autocal] FAILED`，沿用默认 offset。
...
sp 2000 50 0 0         # Kp=2.0 Ki=0.05（与 STM32 demo 同量级思路）
...
角度环 **直接输出 PWM**，含 Kd 微分先行（等效 STM32 角度 D 项）。
...
速度测量经 `APP_BALANCE_SPEED_LPF_ALPHA=0.15` 低通；输入/目标均除以 `APP_BALANCE_SPEED_CPS_SCALE=10` 归一化。
...
差分输出限幅 ±`APP_BALANCE_YAW_MAX_CORRECTION_PM`（默认 200‰）。

**K230 转向指令**：`target_omega` → `target_yaw_pm`；非 0 时航向环 **暂停闭环**（预留后续扩展为改目标航向）。
...
| 能站但慢漂 | pitch offset、速度 I | 校零点；微调 `sp` Ki |
| 直行绕圈 | 航向环、`yi` 极性 | 开 `yp`；查 `yawErr`/`yawCorr` |
...
```text
日期/电池电压：
Pitch invert：1    Yaw invert：1    Speed invert：0
...
2. `bo` + `bp`：先站稳（STM32 思路：角度 P+D + OutOffset）。
3. `sp`：消前后漂。
4. `yp`：最后锁直行航向。
5. 心跳字段：`pitch` / `tilt*` / `pwm` / `yawErr` / `yawCorr` / `v`。
